            ...
            for dc in [-1, 0, 1]:
                ...
                if 0 <= r < self.size and 0 <= c < self.size:
                    if self.grid[r][c] == 'S':
                        return True
        return False
    def mark_area_around_ship(self, ship: Ship):
        """Помечает область вокруг потопленного корабля"""
        for row, col in ship.positions:
            for dr in [-1, 0, 1]:
                for dc in [-1, 0, 1]:
                    r, c = row + dr, col + dc
                    if 0 <= r < self.size and 0 <= c < self.size:
                        if self.grid[r][c] == '~':  # Только вода
                            self.grid[r][c] = 'O'  # Помечаем как промах
    def receive_attack(self, row: int, col: int) -> Tuple[bool, bool, Optional[Ship]]:
        """Возвращает (попал, потопил, корабль)"""
